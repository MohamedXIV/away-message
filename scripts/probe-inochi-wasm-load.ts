...
const EMPTY08_BASE64 = ``;

interface AllocatingWasmApi { nu_malloc(size: number): number | bigint; }
...
interface RuntimeProof extends MutationProof { readonly reloadMatched: boolean; }
...
export function allocatePuppetInputOrThrow(api: AllocatingWasmApi, byteLength: number): number {
  ...
  if (!Number.isSafeInteger(pointer) || pointer <= 0) throw new Error(`Inochi WASM allocator refused ${byteLength} bytes; refusing to write puppet input at address 0 or grow exported memory manually`);
  return pointer;
}
export function assertAcceptedWasmHash(actualHash: string, allowCandidate: boolean): void {
  if (!allowCandidate && actualHash !== PINNED_WASM_SHA256) throw new Error(`unexpected wasm sha256 ${actualHash}; expected ${PINNED_WASM_SHA256} from ${PINNED_UPSTREAM_COMMIT}`);
}
export function resolveProbeFixture(args: readonly string[]): ProbeFixture {
  ...
  if (!fixturePath || fixturePath.startsWith('--')) throw new Error('--fixture requires a puppet file path');
...
export function readWasmCString(memory: WebAssembly.Memory, pointer: number): string {
  if (!pointer) return '';
  const bytes = new Uint8Array(memory.buffer); let end = pointer;
...
function readWasmFloatArray(memory: WebAssembly.Memory, pointer: number, count: number): number[] {
  if (!pointer || count === 0) return [];
  const view = new DataView(memory.buffer, pointer); const values: number[] = [];
  for (let index = 0; index < count; index += 1) values.push(view.getFloat32(index * 4, true));
  return values;
}
...
export function validateParameterSample(name: string, dimensions: number, lower: readonly number[], upper: readonly number[], value: readonly number[]): void {
  if (lower.length !== dimensions || upper.length !== dimensions || value.length !== dimensions) throw new Error(`${name}: real parameter arrays did not match reported dimensionality`);
  for (let index = 0; index < dimensions; index += 1) {
    const low = lower[index]; const high = upper[index]; const current = value[index];
    if (![low, high, current].every(Number.isFinite)) throw new Error(`${name}: non-finite real parameter value/bounds at dimension ${index}`);
    if ((low as number) > (high as number)) throw new Error(`${name}: lower bound exceeds upper bound at dimension ${index}`);
  }
}
export function bootstrapAndAllocatePuppetInput(api: LoadReadyWasmApi, byteLength: number): BootstrappedPuppetInput {
  ...
  return { sourcePointer: allocatePuppetInputOrThrow(api, byteLength), scratchpad };
}
function sha256(bytes: Uint8Array): string { return createHash('sha256').update(bytes).digest('hex'); }
function wasiImports(): WebAssembly.Imports {
  const ok = (): number => 0;
  return { wasi_snapshot_preview1: { fd_close: ok, fd_fdstat_get: ok, fd_fdstat_set_flags: ok, fd_prestat_get: ok, fd_prestat_dir_name: ok, fd_read: ok, fd_seek: ok, fd_write: ok, path_open: ok, proc_exit: ok } };
}
function withCountPointer<T>(api: InochiWasmExports, read: (countPtr: number) => T): { value: T; count: number } {
  ...
  try { new DataView(api.memory.buffer).setUint32(countPtr, 0, true); const value = read(countPtr); return { value, count: new DataView(api.memory.buffer).getUint32(countPtr, true) }; }
  finally { api.nu_free(countPtr); }
}
function enumerateNodes(api: InochiWasmExports, rootPtr: number): { count: number; rootName: string } {
  const pending = [rootPtr]; const seen = new Set<number>(); let rootName = '';
  while (pending.length > 0) {
    const nodePtr = pending.pop() as number; if (!nodePtr || seen.has(nodePtr)) continue; seen.add(nodePtr);
    const name = readWasmCString(api.memory, Number(api.in_node_get_name(nodePtr))); readWasmCString(api.memory, Number(api.in_node_get_type(nodePtr))); if (nodePtr === rootPtr) rootName = name;
    const { value: childrenPtr, count } = withCountPointer(api, (countPtr) => Number(api.in_node_get_children(nodePtr, countPtr))); pending.push(...readWasmPointerArray(api.memory, childrenPtr, count));
  }
...
function proveInventory(api: InochiWasmExports, puppetPtr: number): InventoryProof {
  const name = readWasmCString(api.memory, Number(api.in_puppet_get_name(puppetPtr))); const author = readWasmCString(api.memory, Number(api.in_puppet_get_author(puppetPtr))); const parameters = parameterPointers(api, puppetPtr);
...
function proveParameterReads(api: InochiWasmExports, puppetPtr: number): ParameterReadProof {
  ...
  for (const parameterPtr of parameters.pointers) {
    if (samples.length >= 2) break; const dimensions = api.in_parameter_get_dimensions(parameterPtr); if (dimensions <= 0) continue;
    const name = readWasmCString(api.memory, Number(api.in_parameter_get_name(parameterPtr)));
    const lower = readWasmFloatArray(api.memory, Number(api.in_parameter_get_lower_bounds(parameterPtr)), dimensions); const upper = readWasmFloatArray(api.memory, Number(api.in_parameter_get_upper_bounds(parameterPtr)), dimensions); const value = readWasmFloatArray(api.memory, Number(api.in_parameter_get_value(parameterPtr)), dimensions);
    validateParameterSample(name, dimensions, lower, upper, value); samples.push({ name, dimensions, lower, upper, value });
  }
...
function mutateTwoParameters(api: InochiWasmExports, puppetPtr: number): string[] {
  const parameters = parameterPointers(api, puppetPtr); const mutated: string[] = [];
  for (const parameterPtr of parameters.pointers) {
    if (mutated.length >= 2) break; const dimensions = api.in_parameter_get_dimensions(parameterPtr); if (dimensions <= 0) continue;
    const name = readWasmCString(api.memory, Number(api.in_parameter_get_name(parameterPtr))); const original = readWasmFloatArray(api.memory, Number(api.in_parameter_get_value(parameterPtr)), dimensions); const lower = readWasmFloatArray(api.memory, Number(api.in_parameter_get_lower_bounds(parameterPtr)), dimensions); const upper = readWasmFloatArray(api.memory, Number(api.in_parameter_get_upper_bounds(parameterPtr)), dimensions); validateParameterSample(name, dimensions, lower, upper, original);
    const target = original.map((value, index) => { const low = lower[index] ?? value; const high = upper[index] ?? value; if (Math.abs(value - low) > 1e-5) return low; if (Math.abs(value - high) > 1e-5) return high; return value; });
    ...
    try {
      writeWasmFloatArray(api.memory, valuesPtr, target); api.in_parameter_set_value(parameterPtr, valuesPtr); const observed = readWasmFloatArray(api.memory, Number(api.in_parameter_get_value(parameterPtr)), dimensions); validateParameterSample(name, dimensions, lower, upper, observed); if (!observed.some((value, index) => Math.abs(value - (original[index] ?? value)) > 1e-5)) throw new Error('parameter mutation did not change real WASM state');
      writeWasmFloatArray(api.memory, valuesPtr, original); api.in_parameter_set_value(parameterPtr, valuesPtr); const restored = readWasmFloatArray(api.memory, Number(api.in_parameter_get_value(parameterPtr)), dimensions); validateParameterSample(name, dimensions, lower, upper, restored); if (restored.some((value, index) => Math.abs(value - (original[index] ?? value)) > 1e-5)) throw new Error('parameter restore did not recover original real WASM state');
    } finally { api.nu_free(valuesPtr); }
    mutated.push(name);
  }
...
function proveMutation(api: InochiWasmExports, puppetPtr: number): MutationProof { const parameterReadProof = proveParameterReads(api, puppetPtr); return { ...parameterReadProof, mutatedParameters: mutateTwoParameters(api, puppetPtr) }; }
function loadPuppetBytes(api: InochiWasmExports, puppetBytes: Uint8Array): number {
  ...
  try { new Uint8Array(api.memory.buffer, sourcePtr, puppetBytes.byteLength).set(puppetBytes); return Number(api.in_puppet_load_from_memory(sourcePtr, puppetBytes.byteLength, 0)); }
  finally { api.nu_free(sourcePtr); }
}
function proveRuntime(api: InochiWasmExports, puppetPtr: number, puppetBytes: Uint8Array): RuntimeProof {
  const mutationProof = proveMutation(api, puppetPtr); api.in_puppet_free(puppetPtr); const reloadPtr = loadPuppetBytes(api, puppetBytes); if (!reloadPtr) throw new Error('real puppet failed deterministic reload after dispose'); let reloadMatched = false;
  try { const reloadedName = readWasmCString(api.memory, Number(api.in_puppet_get_name(reloadPtr))); const { count: reloadParameterCount } = withCountPointer(api, (countPtr) => Number(api.in_puppet_get_parameters(reloadPtr, countPtr))); reloadMatched = reloadedName === mutationProof.name && reloadParameterCount === mutationProof.parameterCount; if (!reloadMatched) throw new Error('reloaded puppet metadata/parameter inventory changed'); }
...
async function main(): Promise<void> {
  ...
  try { scratchpad = bootstrapInochiWebModule(api).scratchpad; } catch (error) { console.log(JSON.stringify({ ...baseEvidence, blockedAt: 'bootstrap', requestedBytes: puppetBytes.byteLength, error: error instanceof Error ? error.message : String(error) }, null, 2)); if (expectBlocked) return; throw error; }
  let puppetPtr = 0;
  try { puppetPtr = loadPuppetBytes(api, puppetBytes); } catch (error) { console.log(JSON.stringify({ ...baseEvidence, blockedAt: 'input-allocation', requestedBytes: puppetBytes.byteLength, error: error instanceof Error ? error.message : String(error) }, null, 2)); if (expectBlocked) return; throw error; }
  ...
  console.log(JSON.stringify({ ...baseEvidence, scratchpad, puppetPtr }, null, 2)); api.in_puppet_free(puppetPtr);
}
const argvEntry = process.argv[1]; if (argvEntry && import.meta.url === pathToFileURL(argvEntry).href) await main();
