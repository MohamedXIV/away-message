...
import { resolvePcBootState } from './boot/resolvePcBootState';
import { PcBootScreen } from './boot/PcBootScreen';
import {
  Monitor,
  Globe,
  MessageSquare,
...
export const DesktopShell: React.FC = () => {
  ...
  const bootState = resolvePcBootState({ computer, osId: osVersion, inventory });
  ...
  if (bootState !== 'desktop') {
    return (
      <PcBootScreen
        ...
        onPowerOff={() => {
          setComputerPower(false);
        }}
        onBackToRoom={() => switchView('room')}
      />
    );
  }
